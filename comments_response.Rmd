...
I have only a few minor comments.
  ...
  **Ans: The basic idea of uncertainty analysis in pksensi is to exam if the data from the in-vivo experiment can be located in the range of model simulation output. If not, the modeler should examine all possible reasons that cause the problem (e.g., too narrow parameter range or incomplete model structure). However, detailing the parameter adjusted process after uncertainty analysis is out of the scope of this package. The additional explanation can exceed the word limit in the softwareX. Currently, we have an example of uncertainty analysis for the acetaminophen-PBPK model that was provided in the pksensi website (https://nanhung.netlify.app/pksensi/articles/pbpk_apap.html). The step of uncertainty analysis had also been added to the examples section.**
  ...
  **Ans: We revised and added the content in the section of "Uncertainty analysis" and "Sensitivity analysis" to reflect the workflow of figure 1.** 
  ...
  **Ans: We separate the uncertainty and sensitivity analysis into two subsections (2.4 & 2.5) to distinct them.**
...
Subcounts:
    ...
    389+3+0 (1/0/11/2) Subsection: Parameter matrix generation
    146+1+0 (1/0/0/0) Subsection: Modeling
    169+2+0 (1/0/0/0) Subsection: Uncertainty analysis
    122+2+0 (1/0/0/0) Subsection: Sensitivity analysis
    402+2+0 (1/0/3/0) Section: Illustrative Examples
    534+3+0 (1/0/0/0) Section: Impact and conclusions
...
- Minor Comments
  ...
  **Ans: We revised the description as "The third step is to identify the model parameters that will be examed in global SA and create the parameter matrix. After the parameter selection, we have to assign the corresponding value in probability function, such as minimum and maximum in a uniform distribution. After the definition, the users can use rfast99() with the assignments (e.g., parameter names, sample number, parameter distribution, and its corresponding properties) that were defined in the above steps to create parameter matrix."**
